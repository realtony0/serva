...
function RestaurantMenuContent() {
  ...
  const handleDeleteCategory = async (id: string) => {
    const ok = await confirm("Supprimer cette catégorie ?", { confirmText: "Supprimer", variant: "danger" });
    if (!ok) return;
    try {
  ...
  const handleDeleteType = async (id: string) => {
    const ok = await confirm("Supprimer ce type ?", { confirmText: "Supprimer", variant: "danger" });
    if (!ok) return;
    try {
  ...
  const handleDeleteProduct = async (id: string) => {
    const ok = await confirm("Désactiver ce produit ?", { confirmText: "Désactiver", variant: "danger" });
    if (!ok) return;
    try {
