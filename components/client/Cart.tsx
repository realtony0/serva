...
export default function Cart({
  ...
  const handleCheckout = async () => {
    ...
    if (!restaurantId || !tableId) {
      toast.error("Informations de table manquantes. Veuillez scanner le QR code à nouveau.");
      return;
    }

    const itemCount = items.reduce((sum, item) => sum + item.quantity, 0);
    const confirmed = await confirm(
      `${itemCount} article(s) — ${total.toLocaleString('fr-FR')} FCFA`,
      {
    ...
    try {
      ...
      toast.success(
        `Commande envoyée ! ${itemCount} article(s) — ${total.toLocaleString('fr-FR')} FCFA. Vous serez notifié quand elle sera prête.`,
        8000
      );
